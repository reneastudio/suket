...
try {
    ...
    $pdfFileName = 'Surat_Pengantar_Pindah_Domisili.pdf'; // Default PDF name
    ...
    if ($nikPemohon && !empty($nikPemohon)) {
        ...
        if ($user && isset($user['nama_lengkap'])) {
            ...
            $pdfFileName = 'Surat_Pengantar_Pindah_Domisili_' . preg_replace('/[^a-zA-Z0-9_.-]/', '_', $namaLengkapPemohon) . '_' . $nikPemohon . '_' . time() . '.pdf';
        } else {
             $pdfFileName = 'Surat_Pengantar_Pindah_Domisili_NIKNotFound_' . $nikPemohon . '_' . time() . '.pdf';
        }
    } else {
        // Handle jika NIK kosong atau tidak ada
        $nikPemohon = null; // Pastikan NIK null jika kosong untuk database
        $pdfFileName = 'Surat_Pengantar_Pindah_Domisili_NoNIK_' . time() . '.pdf';
    }
    
    $jenisSurat = "Surat Keterangan Pindah Domisili"; // Jenis surat untuk suketdomisili
    ...
    try {
        ...
        // Untuk NIK, jika null, kita perlu bind sebagai NULL secara eksplisit jika kolomnya mengizinkan NULL.
        // Jika kolom NIK NOT NULL, maka pastikan $nikPemohon tidak pernah null atau berikan default value.
        // Asumsi kolom NIK di tabel 'surat' bisa NULL atau string kosong.
    ...
    } catch (Exception $dbExc) { // Menangkap Exception umum, termasuk dari mysqli
...
// Koneksi $conn akan ditutup secara otomatis ketika skrip berakhir, atau bisa ditutup eksplisit jika ada di config.php
// Tidak perlu blok finally khusus untuk $conn->close() di sini karena sudah dihandle oleh config.php atau auto-closure.
?>
