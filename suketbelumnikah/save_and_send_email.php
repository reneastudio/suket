...
try {
    ...
    if ($nikPemohon && !empty($nikPemohon)) {
        ...
        if ($user && isset($user['nama_lengkap'])) {
            ...
            $pdfFileName = 'Surat_Keterangan_Belum_Menikah_' . preg_replace('/[^a-zA-Z0-9_.-]/', '_', $namaLengkapPemohon) . '_' . $nikPemohon . '_' . time() . '.pdf';
        } else {
             $pdfFileName = 'Surat_Keterangan_Belum_Menikah_NIKNotFound_' . $nikPemohon . '_' . time() . '.pdf';
        }
    } else {
        // Handle jika NIK kosong atau tidak ada
        $nikPemohon = null; // Pastikan NIK null jika kosong untuk database
        $pdfFileName = 'Surat_Keterangan_Belum_Menikah_NoNIK_' . time() . '.pdf';
    }
    
    $jenisSurat = "Surat Keterangan Belum Menikah"; // Jenis surat untuk suketdomisili
