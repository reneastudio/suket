...
function formatTanggalMeninggal(tanggal) {
    if (!tanggal) return "";
...
window.onload = function () {
    ...
    if (dataString) {
        ...
        setValueByClassName('alamat', dataArray[5]);
        setValueByClassName('rt', dataArray[6] ? formatRTRW(dataArray[6]) : '');
        setValueByClassName('rw', dataArray[7] ? formatRTRW(dataArray[7]) : '');
        setValueByClassName('dusun', dataArray[8]);
        setValueByClassName('desa', dataArray[9]); // Ini akan diisi dari data user
        setValueByClassName('kecamatan', dataArray[10]); // Ini akan diisi dari data user
        setValueByClassName('kabupaten', dataArray[11]); // Ini akan diisi dari data user
        setValueByClassName('propinsi', dataArray[12]); // Ini akan diisi dari data user
        setValueByClassName('nama_usaha', dataArray[13]); // Ini akan diisi dari data user
        setValueByClassName('keterangan_usaha', dataArray[14]);
        setValueByClassName('tanggal_meninggal', formatTanggalMeninggal(dataArray[15]));
...
function convertToPDFAndEmail() {
    ...
    const filename = `Surat_Keterangan_Kematian_${sanitizedNamaLengkap}.pdf`;
